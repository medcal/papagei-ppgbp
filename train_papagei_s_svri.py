...
def train_papagei_s_svri():
    """训练PaPaGei-S sVRI模型用于心率预测"""
    ...
    if WANDB_AVAILABLE:
        try:
            wandb.init(
                ...
                name="papagei-s-svri-heart-rate",
                config={
                    "model": "PaPaGei-S sVRI",
                    "target": "heart_rate",
    ...
    # 为心率预测创建数据集和数据加载器
    train_dataset = PPGDataset(ppg_dir, df_train, case_name, "hr")
    val_dataset = PPGDataset(ppg_dir, df_val, case_name, "hr")
    ...
    print("开始训练PaPaGei-S sVRI模型用于心率预测...")
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, f"{model_save_dir}/papagei_s_svri.pt")
    ...
    # 绘制训练曲线
    print("生成训练曲线图...")
    plot_training_curves(train_losses, val_losses, "PaPaGei-S-sVRI", vis_save_dir)
    ...
    if wandb_enabled:
        if val_predictions and val_targets:
            ...
            # 上传训练曲线图
            try:
                wandb.log({"training_curves": wandb.Image(f'{vis_save_dir}/PaPaGei-S-sVRI_training_curves.png')})
                wandb.log({"prediction_results": wandb.Image(
                    f'{vis_save_dir}/PaPaGei-S-sVRI_Heart Rate (bpm)_predictions.png')})
                if val_ppg_samples:
                    wandb.log(
                        {"ppg_examples": wandb.Image(f'{vis_save_dir}/PaPaGei-S-sVRI_Heart Rate_ppg_examples.png')})
            except Exception as e:
    ...
    # 加载最佳模型进行特征提取
    model_path = f"{model_save_dir}/papagei_s_svri.pt"
    if os.path.exists(model_path):
        checkpoint = torch.load(model_path, map_location=device)
        state_dict = checkpoint['state_dict']

        # 创建用于特征提取的新模型（不含回归头）
        feature_model = ResNet1D(
            in_channels=1,
            base_filters=model_config['base_filters'],
            kernel_size=model_config['kernel_size'],
            stride=model_config['stride'],
