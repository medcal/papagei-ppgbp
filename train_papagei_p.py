...
def train_papagei_p():
    ...
    # 为收缩压预测创建数据集和数据加载器
    train_dataset = PPGDataset(ppg_dir, df_train, case_name, "sysbp")
    val_dataset = PPGDataset(ppg_dir, df_val, case_name, "sysbp")
    ...
    # 绘制训练曲线
    print("生成训练曲线图...")
    plot_training_curves(train_losses, val_losses, "PaPaGei-P", vis_save_dir)
    ...
    if wandb_enabled:
        if val_predictions and val_targets:
            ...
            # 上传训练曲线图
            try:
                wandb.log({"training_curves": wandb.Image(f'{vis_save_dir}/PaPaGei-P_training_curves.png')})
                wandb.log(
                    {"prediction_results": wandb.Image(f'{vis_save_dir}/PaPaGei-P_Systolic BP (mmHg)_predictions.png')})
                if val_ppg_samples:
                    wandb.log({"ppg_examples": wandb.Image(f'{vis_save_dir}/PaPaGei-P_Systolic BP_ppg_examples.png')})
            except Exception as e:
    ...
    # 加载最佳模型进行特征提取
    model_path = f"{model_save_dir}/papagei_p.pt"
